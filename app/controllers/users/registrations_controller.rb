...
class Users::RegistrationsController < Devise::RegistrationsController
  ...
  def edit
    ...
    params[:requested_role] = resource.role
  ...
  def update
    ...
    if resource_updated
      if is_flashing_format? && params[:requested_role]=="standard"
        resource.update!(role: :standard)
        flash_key = update_needs_confirmation?(
          resource, prev_unconfirmed_email) ?
  ...
  def after_update_path_for(current_user)
    if params[:requested_role]=="premium" && current_user.standard?
      new_charge_path
    else
      root_path
    end
